...
<p align="center">
  <strong>A smart discovery platform that helps CSOs and policy actors in Rwanda find, trust, and use gender-related data for evidence-based advocacy.</strong>
</p>
...
GenderLens RW is a Streamlit-based web application designed to serve as a centralized discovery platform for gender-related datasets in Rwanda. Built on data curated by the **National Institute of Statistics of Rwanda (NISR)**, it enables Civil Society Organizations (CSOs), researchers, and policy actors to:
...
| Feature | Description |
|---|---|
| **Smart Discovery** | AI-powered TF-IDF semantic search across study titles and abstracts with fuzzy fallback |
| **Analytics Dashboard** | Interactive charts — study trends by year, resource-type distribution, quality breakdown, and organisation profiles |
| **Data Quality Observatory** | Traffic-light quality scoring with completeness, freshness, and resource indicators |
| **Advocacy Toolkit** | AI-assisted policy brief generation, data export, and full provenance tracking |
...
├── assets/
│   └── style.css              # Custom theme (NISR Blue)
├── data/
│   ├── full/                  # Full production datasets
│   └── sample/                # Sample datasets for development/testing
├── docs/
│   ├── Screenshot/            # Architecture diagrams & screenshots
│   └── video/                 # Platform demo & tutorial videos
│       └── Genderlens_tutor.mp4
...
│   ├── schema_config.py       # Data schema configuration
│   └── pipeline/              # Data ingestion & transformation pipeline
│       ├── ingestion.py
│       ├── transform.py
│       └── quality.py
├── tests/                     # Test suite
...
A guided walkthrough of GenderLens RW is included in the repository at `docs/video/Genderlens_tutor.mp4`. The video is embedded directly on the landing page of the application with autoplay (muted) so users can immediately see the platform in action.

**In the app**, the tour is rendered via:

```python
video_path = Path(__file__).parent / "docs" / "video" / "Genderlens_tutor.mp4"
if video_path.exists():
    st.video(str(video_path), autoplay=True, muted=True)
```

> **Watch locally:** Run `streamlit run app.py` and scroll to the **Platform Tour** section, or open `docs/video/Genderlens_tutor.mp4` directly in any media player.

## License

This project is developed as part of the **GIZ Gender Responsive Budgeting & Resource Discovery Challenge 2026**.



<p align="center">
  <strong>GenderLens RW:</strong> Empowering evidence-based gender advocacy in Rwanda<br>
  Data source: <a href="https://microdata.statistics.gov.rw">NISR Microdata Catalog</a> · <a href="https://genderlab.statistics.gov.rw">NISR Gender Data Lab</a>
</p>
